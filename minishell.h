...
typedef	struct s_signal
{
	...
	volatile sig_atomic_t prompt_printed;
} t_signal;
...
void							handle_ctrl_d(int bytes_read, int line_count,
									char *eof);
...
bool							is_file_name(t_arrlst *words, t_token *tokens,
									int *index);

bool							is_start(t_token *tokens, int *index);

bool							is_redirect_correct(t_token *tokens,
									int *index);
...
void							exec_pipe_to_pipe(int *prev_pipe_fd,
									int *pipe_fd, char **cmd, t_dict *envp);
