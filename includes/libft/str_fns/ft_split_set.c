...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/06 14:15:08 by eamsalem          #+#    #+#             */
/*   Updated: 2025/01/06 14:15:27 by eamsalem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../libft.h"

static void	skip_set_index(char const *str, char *set, int *i)
{
	while (str[*i] && chrsetcmp(str[*i], set))
		(*i)++;
}

static int	skip_word(char const *str, char *set, int i)
{
	while (str[i] && !chrsetcmp(str[i], set))
		i++;
	return (i);
...
static char	*get_word(char const *str, char *set, int *i)
{
	int		j;
	int		word_len;
	char	*word;
